    ...
    async execute(query: GetCurrentOnArtByPartnerQuery): Promise<any> {
        let currOnArt = this.repository
            ...
            .select(
                'sum(CurrentOnART_Total) OnART, isnull( SUM ( On_ART_20_24_F ), 0 ) + isnull( SUM ( On_ART_20_24_M ), 0 ) OnART20_24, isnull( SUM ( On_ART_25_Plus_F ), 0 ) + isnull( SUM ( On_ART_25_Plus_M ), 0 ) OnART25_Plus,' +
                    'isnull( SUM ( On_ART_10_14_F ), 0 ) + isnull( SUM ( On_ART_10_14_M ), 0 ) OnART10_14, isnull( SUM ( On_ART_15_19_F ), 0 ) + isnull( SUM ( On_ART_15_19_M ), 0 ) OnART15_19, isnull(sum(On_ART_1_9), 0) OnART1_9, isnull(sum(On_ART_Under_1), 0) OnARTUnder_1, SDP',
            );
        ...
        if (
            ...
            currOnArt = this.repository
                .createQueryBuilder('f')
                .select(
                    'isnull( SUM ( On_ART_20_24_F ), 0 ) + isnull( SUM ( On_ART_25_Plus_F ), 0 ) + isnull( SUM ( On_ART_10_14_F ), 0 ) + isnull( SUM ( On_ART_15_19_F ), 0 ) OnART, f.SDP,' +
            ...
            currOnArt = this.repository
                .createQueryBuilder('f')
                .select(
                    'isnull( SUM ( On_ART_10_14_M ), 0 ) + isnull( SUM ( On_ART_15_19_M ), 0 ) + isnull( SUM ( On_ART_25_Plus_M ), 0 ) + isnull( SUM ( On_ART_20_24_M ), 0 ) OnART, f.SDP,' +
