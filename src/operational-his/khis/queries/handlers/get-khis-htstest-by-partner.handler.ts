...
import { GetKhisHTSTESTByPartnerQuery } from '../impl/get-khis-htstest-by-partner.query'

@QueryHandler(GetKhisHTSTESTByPartnerQuery)
export class GetKhisHTSTESTByPartnerHandler
    implements IQueryHandler<GetKhisHTSTESTByPartnerQuery> {
    constructor(
        @InjectRepository(FactHtsDhis2, 'mssql')
        private readonly repository: Repository<FactHtsDhis2>,
    ) {}

    async execute(query: GetKhisHTSTESTByPartnerQuery): Promise<any> {
