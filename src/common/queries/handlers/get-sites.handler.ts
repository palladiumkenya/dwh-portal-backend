...
export class GetSitesHandler implements IQueryHandler<GetSitesQuery> {
    ...
    async execute(query: GetSitesQuery): Promise<any> {
        const facilities = this.repository.createQueryBuilder('q')
            .select(['max(q.facilityId) mfl, q.name facility, max(q.county) county, max(q.subCounty) subCounty, max(q.agency) agency, max(q.partner) partner, max(q.ownership) owner, max(q.isCt) isCt, max(q.isHts) isHts, max(q.isPkv) isPkv'])
