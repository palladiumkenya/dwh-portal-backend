...
export class GetOtzOutcomesAmongAlhivWithBaselineVlHandler implements IQueryHandler<GetOtzOutcomesAmongAlhivWithBaselineVlQuery> {
    ...
    async execute(query: GetOtzOutcomesAmongAlhivWithBaselineVlQuery): Promise<any> {
        const baselineVl = this.repository.createQueryBuilder('f')
            ...
            'AlHivWithVlLessThan1000 = (SELECT COUNT(*) FROM [dbo].[FACT_Trans_OTZEnrollments] WHERE (CASE WHEN FirstVL = \'undetectable\' THEN 1 WHEN FirstVL = \'DETECTED\' Then 0 ELSE CONVERT(decimal, FirstVL) END) < 1000  AND OTZEnrollmentDate is not null),\n' +
            'AlHivWithVlGreaterThan1000 = (SELECT COUNT(*) FROM [dbo].[FACT_Trans_OTZEnrollments] WHERE (CASE WHEN FirstVL = \'undetectable\' THEN 1 WHEN FirstVL = \'DETECTED\' Then 0 ELSE CONVERT(decimal, FirstVL) END) >= 1000 AND OTZEnrollmentDate is not null)\n'])
