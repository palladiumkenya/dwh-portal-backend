...
export class GetTreatmentOutcomesByFacilityHandler
    implements IQueryHandler<GetTreatmentOutcomesByFacilityQuery> {
    constructor(
    ...
    async execute(query: GetTreatmentOutcomesByFacilityQuery): Promise<any> {
        ...
        const treatmentOutcomes = this.repository
            ...
            .select([
                'f.ARTOutcomeDescription artOutcome, SUM(f.TotalOutcomes) totalOutcomes, MAX(f.County) county, MAX(f.Subcounty) subCounty, f.FacilityName facility, MAX(f.PartnerName) partner',
            ])
        ...
        return await treatmentOutcomes
            .groupBy('f.FacilityName, f.ARTOutcomeDescription')
            .orderBy('f.ARTOutcomeDescription, f.FacilityName')
            .getRawMany();
    }
}
