...
export class GetTreatmentOutcomesByCountyHandler
    implements IQueryHandler<GetTreatmentOutcomesByCountyQuery> {
    constructor(
    ...
    async execute(query: GetTreatmentOutcomesByCountyQuery): Promise<any> {
        ...
        const treatmentOutcomes = this.repository
            ...
            .select([
                'ARTOutcomeDescription artOutcome, SUM(TotalOutcomes) totalOutcomes, County county',
            ])
        ...
        return await treatmentOutcomes
            .groupBy('f.County, f.ARTOutcomeDescription')
            .orderBy('f.ARTOutcomeDescription, f.County')
            .getRawMany();
    }
}
