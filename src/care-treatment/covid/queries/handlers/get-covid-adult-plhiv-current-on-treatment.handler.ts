...
export class GetCovidAdultPLHIVCurrentOnTreatmentHandler implements IQueryHandler<GetCovidAdultPLHIVCurrentOnTreatmentQuery> {
    ...
    async execute(query: GetCovidAdultPLHIVCurrentOnTreatmentQuery): Promise<any> {
        ...
        if (query.partner) {
            covidAdultsCurrentOnTreatment.andWhere('f.PartnerName IN (:...partners)', { partners: query.partner });
        }

        if (query.agency) {
            covidAdultsCurrentOnTreatment.andWhere('f.AgencyName IN (:...agencies)', { agencies: query.agency });
        }
