...
export class GetVlOverallUptakeAndSuppressionLdlHandler implements IQueryHandler<GetVlOverallUptakeAndSuppressionLdlQuery> {
    ...
    async execute(query: GetVlOverallUptakeAndSuppressionLdlQuery): Promise<any> {
        const vlOverallUptakeAndSuppressinLDL = this.repository.createQueryBuilder('f')
        ...
        if (query.county) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.County IN (:...counties)', {counties: query.county});
        }

        if (query.subCounty) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.SubCounty IN (:...subCounties)', {subCounties: query.subCounty});
        }

        if (query.facility) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.FacilityName IN (:...facilities)', {facilities: query.facility});
        }

        if (query.partner) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.CTPartner IN (:...partners)', {partners: query.partner});
        }

        if (query.agency) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.CTAgency IN (:...agencies)', {agencies: query.agency});
        }

        if (query.datimAgeGroup) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.AgeGroup IN (:...ageGroups)', {ageGroups: query.datimAgeGroup});
        }

        if (query.gender) {
            vlOverallUptakeAndSuppressinLDL.andWhere('f.Gender IN (:...genders)', {genders: query.gender});
        }
