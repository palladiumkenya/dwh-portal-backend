...
export class GetDsdStabilityStatusByAgeSexHandler implements IQueryHandler<GetDsdStabilityStatusByAgeSexQuery> {
    ...
    async execute(query: GetDsdStabilityStatusByAgeSexQuery): Promise<any> {
        ...
        if (query.datimAgeGroup) {
            dsdStabilityStatusByAgeSex.andWhere('f.DATIM_AgeGroup IN (:...ageGroups)', { ageGroups: query.datimAgeGroup });
        }

        if (query.gender) {
            dsdStabilityStatusByAgeSex.andWhere('f.Gender IN (:...genders)', { genders: query.gender });
        }
