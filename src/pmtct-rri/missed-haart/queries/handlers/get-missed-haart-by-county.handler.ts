...
import { GetMissedHAARTByCountyQuery } from '../impl/get-missed-haart-by-county.query';

@QueryHandler(GetMissedHAARTByCountyQuery)
export class GetMissedHAARTByCountyHandler
    implements IQueryHandler<GetMissedHAARTByCountyQuery> {
    constructor(
    ...
    async execute(query: GetMissedHAARTByCountyQuery): Promise<any> {
        let missedHAART = this.repository
            ...
            .select(`SUM(KnownPositives) known, SUM(New) new, County`);
