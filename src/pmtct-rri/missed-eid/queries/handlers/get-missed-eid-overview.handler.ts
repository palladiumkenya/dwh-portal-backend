...
import { GetMissedEIDOverviewQuery } from '../impl/get-missed-eid-overview.query';
import { MissedEIDTesting } from '../../entities/missed-eid-testing.model';

@QueryHandler(GetMissedEIDOverviewQuery)
export class GetMissedEIDOverviewHandler
    implements IQueryHandler<GetMissedEIDOverviewQuery> {
    constructor(
        @InjectRepository(MissedEIDTesting, 'mssql')
        private readonly repository: Repository<MissedEIDTesting>,
    ) {}

    async execute(query: GetMissedEIDOverviewQuery): Promise<any> {
        let missedEID = this.repository
            .createQueryBuilder('f')
            .select(
                `SUM(TotalHEI) totalHEI, SUM(Lessthan2Months) lesst2Months, SUM(MissingPCRTests) missingPCRTests`,
            );
